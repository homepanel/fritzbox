...
import com.homepanel.fritzbox.service.EventPublisher;
import org.quartz.*;
...
public class CallMonitor extends Thread {
    ...
    private final static Logger logger = LoggerFactory.getLogger(CallMonitor.class);
    ...
    protected CallMonitorThread monitorThread;
    ...
    protected PhonebookManager phonebookManager;

    // Providers to be able to extract all required items

    protected static CallMonitor instance;
     ...
     */
    public CallMonitor(String url, EventPublisher ep, PhonebookManager pbm) {
        this.eventPublisher = ep;
        this.ip = parseIpFromUrl(url);
        this.port = _DEFAULT_MONITOR_PORT;
        this.phonebookManager = pbm;
        instance = this;
    }
    ...
    public static class ReconnectJob implements Job {
        ...
        public void execute(JobExecutionContext arg0) throws JobExecutionException {
            ...
            instance.monitorThread = instance.new CallMonitorThread();
                ...
                if (ip != null) {
                    ...
                    try {
                        logger.info("Callmonitor Thread [{}] attempting connection to FritzBox on {}:{}..",
                                Thread.currentThread().getId(), ip, port);
                    ...
                    if (reader != null) {
                        logger.info("Connected to FritzBox on {}:{}", ip, port);
                        while (!_interrupted) {
                            try {
        ...
        private void handleCallEvent(CallEvent ce) {
            ...
            if (phonebookManager != null) {
                ...
                if (ce.getExternalNo() == null || ce.getExternalNo().isEmpty()) {
                    ...
                    callerName = phonebookManager.getNameFromNumber(ce.getExternalNo(), 7);
                    if (callerName == null) {
